...
//    System.out.println(SerializeSupport.prettyPrintXML(e));
    ...
    final String decryptedMsg = this.decrypt(encryptedData, new ClassPathResource("credentials/litsec_auth.jks"), "secret",
      "litsec_ab");
...
//    System.out.println(SerializeSupport.prettyPrintXML(e));
  ...
  public void testPeerCapabilities() throws Exception {
    ...
    final EntityDescriptor ed = this.createMetadata(
      new MetadataCertificate("credentials/litsec_auth.crt", UsageType.ENCRYPTION, 
        Arrays.asList(EncryptionConstants.ALGO_ID_BLOCKCIPHER_AES256_GCM, EncryptionConstants.ALGO_ID_KEYTRANSPORT_RSA15)),
      new MetadataCertificate("credentials/litsec_sign.crt", UsageType.SIGNING)); 

    final MetadataResolver resolver = this.createMetadataResolver(ed);
    
    // RSA 1.5 is black-listed in the default OpenSAML config, so we make our own config.
    final BasicEncryptionConfiguration customConfig = DefaultSecurityConfigurationBootstrap.buildDefaultEncryptionConfiguration();
...
//    System.out.println(SerializeSupport.prettyPrintXML(e));
  ...
  private static class MetadataCertificate {
    ...
    public MetadataCertificate(final String certificate, final UsageType usageType, final List<String> encryptionMethods) throws Exception {
    ...
    public KeyDescriptor toKeyDescriptor() throws Exception {
      ...
      final String encoding = Base64.getEncoder().encodeToString(certificate.getEncoded());
      ...
      if (this.encryptionMethods != null) {
        for (String algo : this.encryptionMethods) {
