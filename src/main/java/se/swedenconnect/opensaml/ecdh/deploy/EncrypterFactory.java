...
import se.swedenconnect.opensaml.ecdh.xmlsec.encryption.support.ExtendedDecrypter;
import se.swedenconnect.opensaml.ecdh.xmlsec.encryption.support.ExtendedEncrypter;
...
public class EncrypterFactory {
  ...
  /**
   * Obtains an encrypter for a specific recipient certificate holding the recipients public key using hte factory's default xml encryption parameters.
   * @param receiverCertificate Recipient public key certificate
   ...
   */
  public Encrypter getEncrypter(java.security.cert.X509Certificate receiverCertificate) throws KeyException, NoSuchAlgorithmException, IllegalArgumentException {
   ...
   * @param receiverCertificate Recipient public key certificate
   * @param xmlEncryptModel overrides the default xml encryption model with specific encryption parameters
   * @return {@link ExtendedEncrypter} with ECDH capabilities
   * @throws KeyException key exception
   * @throws NoSuchAlgorithmException no such algotithm
   * @throws IllegalArgumentException illegal argument
   */
  public Encrypter getEncrypter(java.security.cert.X509Certificate receiverCertificate, XmlEncryptModel xmlEncryptModel) throws KeyException, NoSuchAlgorithmException, IllegalArgumentException {
    if (xmlEncryptModel == null){
      xmlEncryptModel = defaultModel;
    }

    // Extract the receiver credential from certificate
    Credential receiverCredential = "EC".equals(receiverCertificate.getPublicKey().getAlgorithm()) ? new ECDHCredential(receiverCertificate)
    ...
    if (receiverCredential instanceof ECDHCredential) {
      ...
      ECDHKeyAgreementBase.prepareBasicKeyAgreementParameters((ECDHCredential) receiverCredential, xmlEncryptModel.getConcatKDFHash());
   ...
   * @param receiverCredentials list of decryption credentials
